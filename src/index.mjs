...
async function claimedByAnotherDevice(db, deviceId) { const { results = [] } = await db.prepare('SELECT device_id FROM private_cloud_state WHERE device_id != ? LIMIT 1').bind(META).all(); return results.some(row => row?.device_id && row.device_id !== deviceId); }
